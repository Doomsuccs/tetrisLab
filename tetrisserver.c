...
int main() {
	...
	// Makes current Client Socket a non-blocking socket
	...
	while(GameOn){
		...
		int status = recv(client_socket, &c, sizeof(c), 0);
		if (status > 0) { // If the client presses a key
			ManipulateCurrent(c);
			refresh();
		}
		else if (status == 0) { // If the client disconnects
